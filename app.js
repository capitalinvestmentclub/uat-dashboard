...
async function init(){
  try{
    ...
    if(delivery){
      ...
      section.innerHTML=`<h2>Historical defect-batch delivery snapshot</h2><p>Source-report release status at publication time; the completed deployed retest above supersedes its pending-UAT wording.</p><p><strong>${escape(delivery.state)}</strong></p><p>${escape(delivery.reportEntriesLinked)} report entries were linked to this batch; ${escape(delivery.uatStatusesChanged)} UAT finding statuses changed in those source-report publications. Recorded in ${deliveryReports} detailed reports.</p><p><strong>${escape(delivery.acceptanceBoundary)}</strong></p>${(delivery.repositories||[]).map(repository=>`<details><summary><span class="row-title">${escape(repository.name)}<small>${escape(repository.deliveryState)} · historical deployed UAT ${escape(repository.uatState)}</small></span></summary><div class="detail"><p><strong>Tests:</strong> ${escape(repository.testState)}</p><p>${escape(repository.note)}</p><p><a href="${safeLink(repository.pullRequest)}">Pull request</a> · <a href="${safeLink(repository.deploymentRun)}">Deployment</a>${repository.postMergeTestRun?` · <a href="${safeLink(repository.postMergeTestRun)}">Post-merge tests</a>`:''}${repository.candidateUrl?` · <a href="${safeLink(repository.candidateUrl)}">Candidate</a>`:''}</p></div></details>`).join('')}`;
      $('deployments').insertAdjacentElement('beforebegin',section);
    }
    if(repairs.length){
    ...
    function render(){
      ...
      $('count').textContent=`${rows.length} ${findings?`${defectsOnly?'unique defects':'unique findings'} · result means latest available finding retest`:'scenarios · result means original campaign outcome'}`;
