...
async function init(){
  try{
    ...
    if(deployedRetest){
      ...
      section.innerHTML=`<h2>Deployed defect-delivery retest</h2><p><strong>${escape(deployedRetest.state)}</strong> · ${escape(deployedRetest.total)} of ${escape(deployedRetest.total)} entries have terminal outcomes in ${escape(deployedRetest.browser)}.</p><p>${escape(outcomes.PASS||0)} passed · ${escape(outcomes.FAIL||0)} still fail · ${escape(outcomes.DUPLICATE_COVERAGE||0)} duplicate coverage · ${escape(outcomes.PASSED_OVER||0)} passed over after bounded attempts.</p><p>Default viewport ${escape(deployedRetest.defaultViewport)}; responsive findings expanded only where required. <a href="${safeLink(new URL(deployedRetest.ledger,location.href).href)}">Open the complete evidence ledger</a>.</p><div class="severity-table" tabindex="0" role="region" aria-label="Retest disposition by report"><table><caption>Terminal outcomes in the ten updated UAT reports</caption><thead><tr><th scope="col">Report</th><th scope="col">Total</th><th scope="col">PASS</th><th scope="col">FAIL</th><th scope="col">DUPLICATE</th><th scope="col">PASSED OVER</th></tr></thead><tbody>${deployedRetest.groups.map(group=>`<tr><th scope="row">${escape(group.group)}</th><td>${escape(group.total)}</td><td>${escape(group.PASS||0)}</td><td>${escape(group.FAIL||0)}</td><td>${escape(group.DUPLICATE_COVERAGE||0)}</td><td>${escape(group.PASSED_OVER||0)}</td></tr>`).join('')}</tbody></table></div></section>`;
